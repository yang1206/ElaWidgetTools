...
#include "Def.h"
T_IconModel::T_IconModel(QObject *parent) : QAbstractListModel{parent} {
...
QVariant T_IconModel::data(const QModelIndex &index, int role) const {
  if (role == Qt::UserRole) {
    if (!_pIsSearchMode) {
      ...
      switch (_iconTypeFilter) {
      case 0: // 全部
        if (index.row() < _elaIconCount) {
          return QStringList{QString("ElaIcon:") +
                                 _elaIconMetaEnum.key(index.row() + 1),
                             QChar(_elaIconMetaEnum.value(index.row() + 1))};
        } else {
          int remixIndex = index.row() - _elaIconCount;
          return QStringList{QString("RemixIcon:") +
                                 _remixIconMetaEnum.key(remixIndex + 1),
      ...
      case 2: // 仅 RemixIcon
        return QStringList{QString("RemixIcon:") +
                               _remixIconMetaEnum.key(index.row() + 1),
                           QChar(_remixIconMetaEnum.value(index.row() + 1))};
      }
      ...
      if (iconType == "ElaIcon") {
        return QStringList{fullKey, QChar(_elaIconMetaEnum.keyToValue(
                                        iconName.toUtf8().constData()))};
      } else if (iconType == "RemixIcon") {
        return QStringList{fullKey, QChar(_remixIconMetaEnum.keyToValue(
                                        iconName.toUtf8().constData()))};
      }
    }
  }
...
QString T_IconModel::getIconNameFromModelIndex(const QModelIndex &index) const {
  ...
  if (_pIsSearchMode) {
    ...
    if (index.row() < _elaIconCount) {
      iconName =
          QString("ElaIconType::") + _elaIconMetaEnum.key(index.row() + 1);
    } else if (index.row() < _rowCount) {
      int remixIndex = index.row() - _elaIconCount;
      iconName =
          QString("RemixIconType::") + _remixIconMetaEnum.key(remixIndex + 1);
    }
  }
  return iconName;
}

void T_IconModel::setIconTypeFilter(int filterType) {
  beginResetModel();
  _iconTypeFilter = filterType;
  ...
  case 0: // 全部
    ...
    break;
  case 1: // 仅 ElaIcon
    _rowCount = _elaIconCount;
    break;
  case 2: // 仅 RemixIcon
    _rowCount = _remixIconCount;
    break;
  }
  endResetModel();
}